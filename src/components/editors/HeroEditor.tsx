...
import { HeroSchema, Hero } from "@/domain/schemas";
import { schemaToFields, HERO_FIELD_CONFIG } from "@/domain/schemaToFields";
...
const HERO_FIELDS = schemaToFields(HeroSchema, HERO_FIELD_CONFIG);

export function HeroEditor({ initialData, filename, mode, onSave, onNavigateToScribe, onDirtyChange, isNew, onDuplicate }: HeroEditorProps) {
  const [quickTag, setQuickTag] = useState("fix");
  const [reason, setReason] = useState("");
  const [editorTab, setEditorTab] = useState<'edit' | 'history'>('edit');
  const [showDeleteDialog, setShowDeleteDialog] = useState(false);

  // Generate ID for new units if not provided
  const [newHeroId] = useState(() => isNew ? crypto.randomUUID() : "");
          ...
          <form onSubmit={(e) => e.preventDefault()} className="space-y-6">
                   ...
                   >
                    ...
                    {!isNew && onDuplicate && (
                        <Button
                            ...
                            disabled={isSaving}
                            title="Duplicate this hero (enters creation mode)"
                        >
            ...
            {editorTab === 'edit' ? (
              <>
                ...
                <div className="mt-8">
                     <HeroAbilitiesPanel control={form.control} />
                </div>
              </>
            ) : (
              <EntityHistoryPanel
