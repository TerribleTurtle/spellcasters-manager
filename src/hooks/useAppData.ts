...
export function useAppData(currentCategory: string) {
  ...
  // Why: fetchData triggers setState inside a useEffect. React warns if setState
  // fires synchronously during the commit phase. Wrapping in setTimeout(…, 0)
  // pushes it to the next microtask, letting the initial render complete first.
  useEffect(() => {
  ...
  const items = useMemo(() => {
    ...
    const CATEGORY_MAP: Record<
      string,
      (folder: string, entity: BaseEntity) => boolean
    > = {
      all: () => true,
      hero: (folder) => folder === "heroes",
      unit: (folder, entity) =>
        folder === "units" && entity.category === "Creature",
      structure: (folder, entity) =>
        folder === "units" && entity.category === "Building",
    ...
    getRegisteredCategories().forEach((cat) => {
      if (
        !CATEGORY_MAP[cat] &&
        !CATEGORY_MAP[ENTITY_REGISTRY[cat].singularLabel.toLowerCase()]
      ) {
