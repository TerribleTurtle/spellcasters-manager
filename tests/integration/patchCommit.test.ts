...
describe('Integration: Patch Commit Flow', () => {
    beforeEach(() => {
        ...
        // Default: Queue empty
        (fileService.exists as any).mockReturnValue(false); 
        (fileService.readJson as any).mockImplementation((path: string) => {
    ...
    it('should add item to queue then commit it successfully', async () => {
        ...
        const addRes = await request(app)
            .post('/api/queue')
            .send({ change: { target_id: 'u1', name: 'Unit', field: 'name', new: 'Updated' } });
        ...
        (fileService.exists as any).mockReturnValue(true);
        (fileService.readJson as any).mockImplementation((path: string) => {
            if (path.includes('queue.json')) return [{ target_id: 'u1', name: 'Unit' }]; // Queue content
            if (path.includes('patches.json')) return [];
            return [];
        });
    ...
    it('should reject commit if queue is empty', async () => {
        // Mock empty queue
        (fileService.exists as any).mockReturnValue(true);
        (fileService.readJson as any).mockReturnValue([]);
